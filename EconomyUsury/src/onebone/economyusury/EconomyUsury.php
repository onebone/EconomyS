...
class EconomyUsury extends PluginBase implements Listener {
	...
	public function addItem($player, Item $i) {
		...
		foreach($data->Inventory as $key => $item) {
			if($key > 8) {
				if($item["id"] == $i->getId() and $item["Damage"] == $i->getMeta()) {
	...
	public function joinHost($player, $host, $due, Item $guarantee, $money) {
		...
		if($this->getServer()->getPlayerExact($player) instanceof Player) {
			...
			$this->usuryHosts[$host]["players"][$player] = [
					$guarantee->getId(), $guarantee->getMeta(), $guarantee->getCount(), time(), $due * 1200, $money, $tid, time(), $this->usuryHosts[$host][1] * 1200, $tid2
			];
			return true;
		}
		$this->schedule_req[$player][] = [$guarantee->getId(), $guarantee->getMeta(), $guarantee->getCount(), $host, $due * 1200, $money];
		return true;
	}
